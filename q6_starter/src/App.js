...
import { collection, addDoc, doc, updateDoc,setDoc, getDocs, deleteDoc,onSnapshot} from "firebase/firestore";
...
function App() {
  ...
  useEffect(() => {
    const snap = onSnapshot(collection(db, "expenses"),(snapshot)=>{
  ...
  const updateExpense = async (expense) => {
    ...
    try {
      const expenseRef = doc(db, "expenses", expense.id); // Specify the correct document reference
      await updateDoc(expenseRef, expense); // Use updateDoc to update the document
